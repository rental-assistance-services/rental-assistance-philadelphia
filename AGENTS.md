...
**Check whether it already exists before you build it.** Search the repo, the open pull requests
(`gh pr list`), and the recent history (`git log --oneline -20`). If the thing already exists,
change that. Do not add a second version next to it.
...
- **Styles live with the page's existing styles.** Change the existing class or CSS variable so
  the fix lands everywhere it is used. Do not add one-off `style="..."` attributes.
...
| Tests, CI, docs, `package.json`, `robots.txt`, `sitemap.xml`, `CNAME` | **No.** Write `No visible change:` and the reason |
...
- **Fill in the template** (`.github/pull_request_template.md`). The **PR checklist** CI job reads
  ...
  - a page file (`.html`, `.css`, an image) changed and the description has neither pictures
    under `## Before` **and** `## After`, nor a `No visible change:` line.
...
- **No AI signatures.** No "Made with Cursor", "Generated by …", or model co-author lines on
  commits, pull requests, or comments. The work is from the person at the keyboard.
